...
const chordPeakGain = 0.08;
const masterGainFloor = 0.01;
const masterGainCeiling = 0.22;
...
function queueChord(context: AudioContext, masterGain: GainNode, notes: readonly number[]) {
  ...
  gain.gain.exponentialRampToValueAtTime(chordPeakGain, startAt + 0.6);
...
function normalizeVolume(volume: number) {
  return Math.max(masterGainFloor, Math.min(masterGainCeiling, volume / 450));
}
